...
def eval_validation_baseline(
    ...
    # Label ints for metrics (with simple retry)
    try:
...
def main():
    ...
    if args.quick:
        ...
        # Update timesteps in meta to reflect kept positions (for saved artifact metadata)
    ...
    # Baseline (greedy) — FIX: correct tag check
    ...
    feats = arrays[win_key]  # [n, d] (we kept mid_t1,t2 so win_key is present)
    ...
    if len(scores_pos) > 0 and len(scores_neg) > 0:
        # For fast test keep B small; real run can use 500+
