...
from src.label_constr_aware import label_batch, label_batch_full, save_labeled_results, labels_exist, load_final_labels, type5_rate_from_finals
...
def eval_validation_baseline(
    ...
    """
    If cache_dir & cache_tag are provided, tries to load baseline labels from:
      data/intervention/<cache_tag>_labels.jsonl
    Otherwise runs generation+labeling and (if cache_dir given) writes them there.
    """
    if cache_dir and cache_tag and labels_exist(Path(cache_dir), cache_tag):
        finals = load_final_labels(Path(cache_dir), cache_tag)
        rate5, not5, N = type5_rate_from_finals(finals)
        print(f"🔁 Baseline cached: {cache_tag}  N={N}  Type-5={rate5:.3f}")
        # Also return basic list[int] labels if caller expects it:
        labels_int = [r.get("type", 7) for r in finals]
        return rate5, not5, N, labels_int, None, None

    # ... otherwise run as before (your original generation+labeling) ...
    pairs = []
    samples = []
    ...
    if decode_mode == "greedy":
        ...
        for pi, text in enumerate(val_prompts):
            for si in range(num_samples):
                ...
                pairs.append((text, out))
                print(f"\n\nBaseline gen: prompt {pi+1}/{len(val_prompts)} sample {si+1}/{num_samples}\n{text}\n==>\n{out}\n")
                if return_payload:
                    pid = val_ids[pi] if val_ids else f"VAL_{pi}"
                    samples.append({"prompt_id": pid, "sample_idx": si, "text": text, "output_text": out})

    # label ints for metrics
    # if doesn't succeed, retry (up to 6 times) after 10s
    try:
        labels = label_batch(judge_model, pairs, constraint_aware=constraint_aware)
    except Exception as e:
        import time
        print(f"Labeling failed with {e}, retrying up to 5 times after 10s...")
        for attempt in range(5):
            ...
            try:
                ...
                print(f"Retry {attempt+1} failed with {e2}.")
    ...
    if cache_dir and cache_tag:
        if not return_payload:
            # need full records to save; produce now
            samples = []
            if decode_mode == "greedy":
                for pi, (text, out) in enumerate(pairs):
                    pid = val_ids[pi] if val_ids else f"VAL_{pi}"
                    samples.append({"prompt_id": pid, "sample_idx": 0, "text": text, "output_text": out})
            else:
                idx = {}
                ...
                for (text, out) in pairs:
                    si = idx[text]
                    idx[text] += 1
                    pid = val_ids[val_prompts.index(text)] if val_ids else f"VAL_{val_prompts.index(text)}"
                    samples.append({"prompt_id": pid, "sample_idx": si, "text": text, "output_text": out})
        labeled_full = label_batch_full(judge_model, samples, constraint_aware=constraint_aware)
        save_labeled_results(Path(cache_dir), cache_tag, labeled_full, constraint_aware)
...
def eval_validation_rates(
    ...
    """
    If save_dir & run_tag are provided and *_labels.jsonl exists, loads it and returns rates.
    Else, runs generation+labeling and saves to data/intervention/<run_tag>_*.
    """
    if save_dir and run_tag and labels_exist(Path(save_dir), run_tag):
        finals = load_final_labels(Path(save_dir), run_tag)
        rate5, not5, N = type5_rate_from_finals(finals)
        print(f"🔁 Steered cached: {run_tag}  N={N}  Type-5={rate5:.3f}")
        return rate5, not5, N
    ...
    if decode_mode == "greedy":
        ...
        for pi, text in enumerate(val_prompts):
            for si in range(num_samples):
                ...
                pairs.append((text, out))
                print(f"\n\nSteered gen: prompt {pi+1}/{len(val_prompts)} sample {si+1}/{num_samples}\n{text}\n==>\n{out}\n")
                pid = val_ids[pi] if val_ids else f"VAL_{pi}"
                samples.append({"prompt_id": pid, "sample_idx": si, "text": text, "output_text": out})

    try:
        labels = label_batch(judge_model, [(s["text"], s["output_text"]) for s in samples], constraint_aware=constraint_aware)
    except Exception as e:
        import time
        print(f"Labeling failed with {e}, retrying up to 6 times after 10s...")
        for attempt in range(5):
            time.sleep(10)
            try:
                labels = label_batch(judge_model, [(s["text"], s["output_text"]) for s in samples], constraint_aware=constraint_aware)
    ...
    if save_dir and run_tag:
        labeled_full = label_batch_full(judge_model, samples, constraint_aware=constraint_aware)
        save_labeled_results(Path(save_dir), run_tag, labeled_full, constraint_aware)
...
def main():
    ...
    # filter arrays, meta to keep only the tuples with arrays.keys() in [layer_mid_t1, layer_mid_t2]
    ...
    # 2) Candidate × alpha selection — skip when files exist
    ...
    for cand in candidates:
        ...
        for a in alphas:
            ...
            # Load-or-run steered validation
            ...
            if drift_file.exists():
                ...
                drift_file.write_text(json.dumps({"mean_kl": float(mean_kl), "mean_top10": float(mean_top10)}, indent=2), encoding="utf-8")
    ...
    if best_a04["J_multi"] > best_a02["J_multi"]:
    ...
    base_labels_full = None  # ensure defined
    if not labels_exist(INTERV_DIR, add_tag):
        ...
        save_labeled_results(Path("data/intervention"),
                            f"greedy_base",
                            base_labels_full, args.constraint_aware)
    else:
    ...
    add_labels_full = None  # ensure defined
    if not labels_exist(INTERV_DIR, add_tag):
        samples_add = []
        for pi, text in enumerate(val_prompts):
    ...
    abl_labels_full = None  # ensure defined
